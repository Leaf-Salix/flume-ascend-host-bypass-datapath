 ...
 public:
  ...
  bool Open(const HostRaConfig& requested, std::string* error) {
    ...
    if (init_status != cann::kSuccess) {
      if (error != nullptr) {
        *error = "RaInit failed (status=" + std::to_string(init_status) + ")";
      }
      return Fail();
    }
    ra_initialized = true;
  ...
  bool SubmitAndWait(Operation operation, uint64_t request_id, uint64_t object_id,
    ...
    result->marker = MakeHostRaSuccessMarker(config.control_mode,
        ...
        " cann_ra_network_bootstrap=" +
        api.network_bootstrap_profile_name() + " cann_ra_hdc_type=" +
        std::to_string(effective_hdc_type);
    return true;
  }
