...
`payload_local_buffer_prime_source=host-sentinel-not-payload` is intentionally part of the strict gate: the host-written sentinel only guards against stale HCCL Buffer contents before the direct ACL custom-op launch. It is not treated as user payload movement; real payload success still requires the HCOMM primitive trace, checksum, no-HCCL-payload markers, and `fallback=none`.

`payload_trace_operand_layout=...` is derived from the kernel-side trace header after the direct ACL custom-op runs. It records the primitive operand shape, for example `input-hbm->local-hccl-buffer` on the send rank and `remote-hccl-buffer->output-hbm` on the focused official-p2p recv rank, so offline log verification can reject stale logs whose path marker does not match the device-side layout.
