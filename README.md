...
- Stage 3B.3E/3B.3F HCOMM payload-copy candidates. The custom-op package exports the V4 payload ABI plus the current payload semantic v10 write-path-capable marker, and contains a kernel that consumes the Flume descriptor, optionally acquires the HCOMM communicator by HCCL comm name, calls HCOMM primitive APIs, releases the communicator when acquired, then reports device-visible status words, descriptor echo words, descriptor fingerprint, and a device-side primitive trace. The default read-path candidate has rank1 call `HcommReadOnThread(remote HCCL Buffer -> local HCCL Buffer)` before locally copying into user HBM. The write-path candidate, enabled with `--hcomm-payload-write-path`, has rank0 call `HcommWriteOnThread(local HCCL Buffer -> remote HCCL Buffer)` and rank1 locally copy from its HCCL Buffer into user HBM after ready notification. A diagnostic `--hcomm-payload-recv-direct-output` path can instead make the read-path recv call `HcommReadOnThread(remote HCCL Buffer -> output HBM)` directly to compare with the public custom P2P example when isolating recv output-copy failures. These are strict-positive candidates, but still require an installed payload-ready custom-op package and remote NPU evidence before either is considered complete.
