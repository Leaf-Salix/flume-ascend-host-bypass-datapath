...
- Stage 3B.3E HCOMM payload-copy candidate. The custom-op package exports the V3 payload ABI plus the current payload semantic marker, and contains a kernel that consumes the Flume descriptor, acquires the HCOMM communicator by HCCL comm name, calls `HcommLocalCopyOnThread`, `HcommReadOnThread`, and HCOMM Channel Notify primitives, releases the communicator, then reports device-visible status words. This is implemented as a strict-positive candidate, but still requires an installed payload-ready custom-op package and remote NPU evidence before it is considered complete.
