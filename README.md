...
- Stage 3B.3E HCOMM payload-copy candidate. The custom-op package exports the V4 payload ABI plus the current payload semantic v8 ordered-trace-capable marker, and contains a kernel that consumes the Flume descriptor, acquires the HCOMM communicator by HCCL comm name, calls `HcommLocalCopyOnThread`, `HcommReadOnThread`, and HCOMM Channel Notify primitives, releases the communicator, then reports device-visible status words, descriptor echo words, and a device-side primitive trace. The default recv rank reads the peer HCCL Buffer into its local HCCL Buffer first, then performs a kernel-side local copy into user HBM, matching HCOMM's stricter HCCL Buffer memory constraints. A diagnostic `--hcomm-payload-recv-direct-output` path can instead make recv call `HcommReadOnThread(remote HCCL Buffer -> output HBM)` directly to compare with the public custom P2P example when isolating recv output-copy failures. This is implemented as a strict-positive candidate, but still requires an installed payload-ready custom-op package and remote NPU evidence before it is considered complete.
