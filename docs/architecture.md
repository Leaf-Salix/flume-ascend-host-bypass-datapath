...
当前实现采用 push-first storage read，并支持三种部署：data mover node
host RNIC 直写 compute HBM、CPU storage daemon 配合 NPU RA/HCCN data mover、
以及独立 control node 透明转发到真正连接 RoCE fabric 的 data mover node。第三种
部署中 control node 不终止 QP、不访问 payload；详见
[`stage-4-push-routing.md`](stage-4-push-routing.md)。Pull 只保留接口和协议位，
当前显式返回 unsupported。

Storage backend 与 data mover 可以分属不同节点：当前 data mover 可从本地
文件或已挂载的远端 namespace 读取，再使用自身 RNIC 发包。因此网络拓扑
判断应以 data mover RNIC 到 compute NPU HCCN 的可达性为准，而不是 SSD
物理所在节点是否具备 RNIC。
...
| Host-RA push + RNIC/NPU data mover | 已实现并静态验证，待真机 | `ctest -R 'roce_|cann_ra|npu_ra'`；真机见 `docs/stage-4-push-routing.md` | TCP 传 command/completion；RA/HCCP 注册 NPU HBM；host verbs 或 NPU RA sender 执行 RDMA Write；可经透明 control proxy 路由到直连 data node |
...
storage data block
  ...
  -> registered target HBM window / future RDMA MR
...
sequenceDiagram
    ...
    Rank1->>HCOMM1: HcommReadOnThread(remote HCCL Buffer -> local HCCL Buffer)
    Rank1->>HCOMM1: HcommLocalCopyOnThread(local HCCL Buffer -> output HBM)
...
- recv 端：`HcommReadOnThread(localBuffer.addr, remoteBuffer.addr)` 后再 `HcommLocalCopyOnThread(outputPtr, localBuffer.addr)`。
...
- HCOMM probe 默认 `engine=auto`：以 `HcclThreadExportToCommEngine` 的声明+可链接符号为能力依据，声明既可以在 `hccl_res.h`，也可以在可选 `hccl_res_expt.h`。能力成立时选择 `aicpu-ts`，否则选择 `cpu-ts` 且只证明 channel resource path。`--hcomm-require-thread-export` 是严格 AICPU thread-export 前置检查，不再以某个可选头文件缺失作为 unsupported 依据。
...
- 已增加本地 storage direct transfer skeleton：`flume_get_storage_transfer_caps`、`flume_register_storage_target_memory`、`flume_storage_direct_plan_create` 和 `flume_read_storage_to_hbm_async` 可以在 Mac/无 NPU 环境模拟 `storage block -> registered target window -> storage fabric sim -> HCOMM payload sim -> target SIM_HBM`，输出 `storage_direct_sim=on`、`storage_hbm_path=sim-direct`、`storage_fabric=sim-rdma`、`storage_memory_registration=sim-hbm-window`、`storage_host_payload_copy=not-used`、`hcomm_payload_backend=sim`、`fallback=none`；host-staging fallback 会显式输出 `storage_host_payload_copy=used` 和 `fallback=host-staging`；`flume-storage-read-to-rank-demo` 从应用视角验证 `app_request=read_to_rank_hbm`。
...
- 已开始 Stage 3B.3B：launcher router 会探测 public `HcclAicpuKernelLaunch`、direct ACL runtime custom-op launch API、HCOMM primitive、custom-op package 和可选 thread-export 状态；目标是在不同 CANN 暴露能力下稳定选择可用 launcher 或输出精确 unsupported reason。`thread_export=off` 不再阻止 direct ACL payload candidate，只会把 completion marker 降级为 `stream-sync+status-word`。
...
- 已开始 Stage 3B.3E/3B.3F：`flume_hcomm_payload_send_async` / `flume_hcomm_payload_recv_async` 在 strict smoke 下会尝试 direct ACL custom-op payload kernel；`hcomm-payload-strict-positive` 默认选择 official-p2p 形态：`aicpu + channel-handle + no-batch + direct-output + ordered-notify`。该形态下 send rank 执行 `HcommLocalCopyOnThread(input -> local_hccl_buffer)` + ready Notify，recv rank 执行 ready Notify wait + `HcommReadOnThread(remote_hccl_buffer -> output)` + done Notify，不走 `HcclSend` / `HcclRecv` payload API。可选 `--hcomm-payload-write-path` 会让 send rank 额外执行 `HcommWriteOnThread(local_hccl_buffer -> remote_hccl_buffer)`，recv rank 等 ready 后从本地 HCCL Buffer copy 到 output，marker 为 `payload_transfer_mode=write`。可选 `--hcomm-payload-recv-direct-output` 诊断会让 read-path recv rank 改成 `HcommReadOnThread(remote_hccl_buffer -> output)`，用于隔离 output-copy 失败，marker 为 `payload_recv_path=direct-output`。可选 `--hcomm-payload-skip-comm-acquire` 诊断会跳过 kernel 内 `HcommAcquireComm` / `HcommReleaseComm`，用于隔离 comm-name binding 问题，并标记为 `payload_comm_binding=diagnostic-skip`，不能作为最终成功。显式 `--hcomm-payload-comm-binding=channel-handle` 使用已获取的 ThreadHandle/ChannelHandle resource，不在 kernel 内做 comm acquire；它是当前默认 strict-positive 的 official-p2p 资源绑定方式。该路径只有在 strict-positive 同时输出两 rank passed、`stage3b3e_payload_copy=passed`、direct ACL payload launch/sync passed、`payload_kernel_status=success`、`payload_failure_step=none`、`payload_status_word=0`、`payload_kernel_hcomm_ret=0`、`payload_local_buffer_prime=passed`、`payload_local_buffer_prime_source=host-sentinel-not-payload`、`payload_status_schema=v7`、`payload_status_word_count=17`、`payload_echo=passed`, `payload_descriptor_fingerprint=passed`, `payload_host_descriptor_validation=passed`, `payload_host_validation_reason=ok`, `payload_host_validation_reason_code=0`, `payload_data_probe=observed`、`payload_data_remote_entry_fingerprint=...`, `payload_data_transfer_exit_fingerprint=...`、`payload_data_flow=passed`、`payload_host_data=passed`、`payload_primitive_state=completed`、`payload_trace=passed`、`payload_trace_header=passed`、`payload_trace_schema=v3`、`payload_trace_word_count=82`、`payload_trace_status_word=0`、`payload_trace_hcomm_ret=0`、`payload_trace_event=kernel-exit`、`payload_trace_role=...`、`payload_trace_local_rank=...`、`payload_trace_peer_rank=...`、`payload_trace_order=passed`、`payload_trace_ret_order=passed`、`payload_trace_primitive_counts=passed`、`payload_trace_primitive_path=send-local-copy|recv-read-*|send-write|recv-write-local-copy|send-write-with-notify|recv-write-notify-local-copy`、`payload_trace_operand_layout=...`、`payload_transfer_mode=read|write|write-with-notify`、`payload_trace_transfer_mode=read|write|write-with-notify`、`payload_trace_result=success`、`payload_trace_first_error_event=none`、`payload_trace_first_error_ret=0`、`payload_trace_first_error_index=-1`、`payload_comm_binding=channel-handle` 或显式专家 `comm-name`、`payload_desc_batch_tag=default|custom`、`payload_recv_path=local-buffer|direct-output`、`payload_semantic_v6=present`、`payload_semantic_v7=present`、`payload_semantic_v8=present`, `payload_semantic_v9=present`、`payload_semantic_v10=present`, `payload_semantic_v11=present`、`payload_semantic_v12=present`、`payload_semantic_v13=present`、`payload_semantic_v14=present`, `payload_semantic_v15=present`、`payload_semantic_v16=present`, `payload_semantic_v17=present`、`payload_semantic_v18=present`、`payload_semantic_v19=present`、`payload_copy_api=hcomm-direct-aclrt`、`payload_hccl_p2p_api=not-used`、`payload_no_hccl_sendrecv=passed`、`payload_no_hccl_payload_collective=passed`、`payload_thread_notify_order=...`、`payload_pattern=strict-v1`、source/received/expected checksum match、`payload_verify=passed` 和 `fallback=none` 时才算完成。
