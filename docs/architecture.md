...
- 已开始 Stage 3B.3E：`flume_hcomm_payload_send_async` / `flume_hcomm_payload_recv_async` 在 strict smoke 下会尝试 direct ACL custom-op payload kernel；send rank 执行 `HcommLocalCopyOnThread(input -> local_hccl_buffer)` + Notify，recv rank 执行 Notify + `HcommReadOnThread(remote_hccl_buffer -> output)`。该路径只有在 strict-positive 同时输出两 rank passed、`stage3b3e_payload_copy=passed`、direct ACL payload launch/sync passed、`payload_kernel_status=success`、`payload_status_word=0`、`payload_kernel_hcomm_ret=0`、`payload_echo=passed`、`payload_verify=passed` 和 `fallback=none` 时才算完成。
