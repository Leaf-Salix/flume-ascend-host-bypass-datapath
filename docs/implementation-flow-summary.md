...
| Stage 3B.3E HCOMM payload copy | `--build-hcomm-custom-op --run-hcomm-payload-smoke --hcomm-require-payload-copy` | 待真机验证 | 目标 marker 为 `stage3b3e_payload_copy=passed`、direct ACL payload launch passed、fallback none；缺 payload package/function 时应清晰 unsupported |
