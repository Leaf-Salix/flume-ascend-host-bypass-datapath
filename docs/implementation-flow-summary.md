...
| Stage 3B.3E HCOMM payload copy | `--build-hcomm-custom-op --run-hcomm-payload-smoke --hcomm-require-payload-copy` | 待真机验证 | 目标 marker 为两 rank passed、`stage3b3e_payload_copy=passed`、direct ACL payload launch/sync passed、`payload_kernel_status=success`、`payload_status_word=0`、`payload_kernel_hcomm_ret=0`、`payload_echo=passed`、`payload_verify=passed`、`fallback=none`；缺 payload package/function 时应清晰 unsupported |
