...
`payload_trace_order=passed` 和 `payload_trace_result=success`；失败时重点看
`payload_trace_event`、`payload_trace_ret` 和 `payload_trace_sequence`，它们
表示 kernel-side 最后进入或返回的 primitive 事件、该事件的原始返回码和
已执行的 primitive 顺序。
