...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--hcomm-resource-descriptor-smoke", action="store_true",
                        help="Run Stage 3B.2 HCOMM resource descriptor smoke")
    parser.add_argument("--hcomm-notify-only-smoke", action="store_true",
                        help="Run Stage 3B.2-complete HCOMM notify-only smoke")
    ...
    parser.add_argument("--storage-hbm-smoke", action="store_true",
                        help="Run Stage 3A storage proxy HBM smoke")
    parser.add_argument("--storage-smoke-file", default="",
                        help="File slice read by rank0 storage proxy")
    parser.add_argument("--storage-smoke-offset", type=int, default=0)
    parser.add_argument("--storage-smoke-bytes", type=int, default=4096)
    parser.add_argument("--storage-smoke-checksum", default="",
                        help="Expected Checksum32 of the file slice")
    ...
    if (args.p2p_copy or args.hcomm_channel_probe or args.hcomm_custom_op_launch_smoke or args.hcomm_resource_descriptor_smoke or args.hcomm_notify_only_smoke or args.hcomm_payload_smoke or args.storage_hbm_smoke) and len(args.devices_list) != 2:
        parser.error("--p2p-copy, --hcomm-channel-probe, --hcomm-custom-op-launch-smoke, --hcomm-resource-descriptor-smoke, --hcomm-notify-only-smoke, --hcomm-payload-smoke, and --storage-hbm-smoke require exactly two ranks")
...
def build_rank_command(args: argparse.Namespace, rank: int, device: str,
    ...
    if args.hcomm_resource_descriptor_smoke:
        command.append("--hcomm-resource-descriptor-smoke")
    if args.hcomm_notify_only_smoke:
    ...
    if args.storage_hbm_smoke:
        ...
        command.append(f"--storage-smoke-file={args.storage_smoke_file}")
