...
HCOMM_CUSTOM_OP_FUNCTIONS = {
    ...
    "payload_semantic_v17": "FlumeHcommPayloadCopySemanticVersion17",
    "payload_semantic_v18": "FlumeHcommPayloadCopySemanticVersion18",
    "payload_semantic_v19": "FlumeHcommPayloadCopySemanticVersion19",
...
HCOMM_PAYLOAD_COPY_SEMANTIC_VERSION_V17 = "FlumeHcommPayloadCopySemanticVersion17"
HCOMM_PAYLOAD_COPY_SEMANTIC_VERSION_V18 = "FlumeHcommPayloadCopySemanticVersion18"
HCOMM_PAYLOAD_COPY_SEMANTIC_VERSION_V19 = "FlumeHcommPayloadCopySemanticVersion19"
...
HCOMM_PAYLOAD_METADATA_EXPECTED = {
    ...
    "payload_semantic_version": (HCOMM_PAYLOAD_COPY_SEMANTIC_VERSION, 19),
    ...
    "payload_semantic_version_v17": (HCOMM_PAYLOAD_COPY_SEMANTIC_VERSION_V17, 1),
    "payload_semantic_version_v18": (HCOMM_PAYLOAD_COPY_SEMANTIC_VERSION_V18, 1),
    "payload_semantic_version_v19": (HCOMM_PAYLOAD_COPY_SEMANTIC_VERSION_V19, 1),
...
def run_ascend_probe(args: argparse.Namespace) -> int:
    ...
    if args.hcomm_require_payload_copy and package_result is not None:
        ...
        RecordStrictPositiveEvidenceGate(
            runner, tree, DecisionTreeStrictPositiveEvidencePassed(tree),
    ...
    note.write_text(
        ...
        "prerequisite check. The capability probe accepts declarations from "
        "hccl_res.h or the optional hccl_res_expt.h and requires a linkable "
        "HcclThreadExportToCommEngine symbol. Add "
...
def StrictPayloadDataFlowPassed(rank_lines: dict[int, str]) -> tuple[bool, str]:
    ...
    rank0_remote_entry = MarkerValueFromLine(
        rank0_line, "payload_data_remote_entry_fingerprint")
    rank0_transfer_exit = MarkerValueFromLine(
        rank0_line, "payload_data_transfer_exit_fingerprint")
    ...
    rank1_remote_entry = MarkerValueFromLine(
        rank1_line, "payload_data_remote_entry_fingerprint")
    rank1_transfer_exit = MarkerValueFromLine(
        rank1_line, "payload_data_transfer_exit_fingerprint")
    ...
    values = (
        rank0_entry, rank0_local_entry, rank0_remote_entry,
        rank0_transfer_exit, rank0_local_exit, rank0_user_exit, rank0_sample,
        rank1_entry, rank1_local_entry, rank1_remote_entry,
        rank1_transfer_exit, rank1_local_exit, rank1_user_exit, rank1_sample)
    ...
    if (rank0_remote_entry != "not-sampled" and
            rank0_remote_entry == expected_payload):
        return False, "send-remote-entry-already-matched"
    if (rank0_transfer_exit != "not-sampled" and
            rank0_transfer_exit != expected_payload):
...
def StrictPayloadTraceDescriptorPassed(
    ...
    for rank in (0, 1):
        ...
        expected_operand_layout = {
            ...
            "recv-write-local-copy": "local-hccl-buffer->output-hbm",
            "recv-write-notify-local-copy": "local-hccl-buffer->output-hbm",
        }.get(trace_path)
        if expected_operand_layout is None:
...
def WriteHcommPayloadWriteWithNotifyCandidate(
    ...
    semantic_v14 = MarkerValue(candidate_text, "payload_semantic_v14")
    ...
    if passed and transfer_mode == "write-with-notify":
        ...
        next_action = (
            "rebuild/reinstall the payload custom-op package from this commit")
    elif semantic_v14 == "missing":
        decision = (
            "write-with-notify candidate package is missing the v14 semantic "
            "export")
        next_action = (
            "rebuild/reinstall the payload custom-op package from this commit")
...
def WriteMatrixDecisionTree(run_dir: Path, smoke_log: Optional[Path],
    ...
    strict_semantic_v18 = marker_value(strict, "payload_semantic_v18")
    strict_semantic_v19 = marker_value(strict, "payload_semantic_v19")
    ...
    strict_rank0_data_remote_entry = marker_value_from_line(
        strict_rank_lines[0], "payload_data_remote_entry_fingerprint")
    strict_rank0_data_transfer_exit = marker_value_from_line(
        strict_rank_lines[0], "payload_data_transfer_exit_fingerprint")
    ...
    strict_rank1_data_remote_entry = marker_value_from_line(
        strict_rank_lines[1], "payload_data_remote_entry_fingerprint")
    strict_rank1_data_transfer_exit = marker_value_from_line(
        strict_rank_lines[1], "payload_data_transfer_exit_fingerprint")
    ...
    strict_rank0_write_notify_selected = marker_value_from_line(
        strict_rank_lines[0], "payload_write_notify_backend")
    strict_rank1_write_notify_selected = marker_value_from_line(
        strict_rank_lines[1], "payload_write_notify_backend")
    strict_rank0_layout = marker_value_from_line(
        strict_rank_lines[0], "payload_layout")
    strict_rank1_layout = marker_value_from_line(
        strict_rank_lines[1], "payload_layout")
    ...
    for rank in (0, 1):
        ...
        rank_status[rank] = {
            ...
            "validation_reason": marker_value_from_line(
                rank_line, "payload_validation_reason"),
            "validation_reason_code": marker_value_from_line(
                rank_line, "payload_validation_reason_code"),
    ...
    if strict_log is not None:
        lines.extend([
            ...
            f"| payload semantic v18 marker | {strict_semantic_v18} | `payload_semantic_v18=missing` means the package predates descriptor primitive/operand layout evidence gates |",
            f"| payload semantic v19 marker | {strict_semantic_v19} | `payload_semantic_v19=missing` means the package predates descriptor validation reason evidence |",
    ...
    if strict_positive_ok:
        ...
        if strict_resource_step != "missing":
            ...
            elif strict_semantic_v17 == "missing":
                next_action = (
                    "rebuild/reinstall payload custom-op package with current "
                    "Flume semantic v17 remote-buffer-safe data-probe-capable "
                    "kernel")
            elif strict_semantic_v18 == "missing":
                next_action = (
                    "rebuild/reinstall payload custom-op package with current "
                    "Flume semantic v18 descriptor-layout evidence markers")
            elif strict_semantic_v19 == "missing":
                next_action = (
                    "rebuild/reinstall payload custom-op package with current "
                    "Flume semantic v19 descriptor validation reason markers")
...
def DecisionTreeStrictPositiveEvidencePassed(tree_path: Path) -> bool:
    try:
        text = tree_path.read_text(encoding="utf-8", errors="replace")
    except OSError:
        return False
...
def DecisionTreeHcommStoragePassed(tree_path: Path) -> bool:
    try:
        ...
        return False
    return (
        DecisionTreeStrictPositiveEvidencePassed(tree_path) and
...
def AnalyzeHcommPayloadStrictPositiveLogs(
    ...
    if not DecisionTreeStrictPositiveEvidencePassed(tree):
        ...
        if channel_log is not None:
            ...
            return (
                channel_tree,
                DecisionTreeStrictPositiveEvidencePassed(channel_tree),
                smoke_log, channel_log, package_log)
    return (tree, DecisionTreeStrictPositiveEvidencePassed(tree), smoke_log,
...
def run_hcomm_storage_strict_positive(args: argparse.Namespace) -> int:
    ...
    strict_passed = DecisionTreeStrictPositiveEvidencePassed(tree)
...
def run_hcomm_custom_op_package(args: argparse.Namespace) -> int:
    ...
    found_payload_semantic_v17_marker = False
    found_payload_semantic_v18_marker = False
    found_payload_semantic_v19_marker = False
    ...
    for root, vendor, json_path, tar_path in HcommCustomOpPackageCandidates(
        ...
        symbol_names = list(HCOMM_CUSTOM_OP_FUNCTIONS.values()) + [
            ...
            HCOMM_PAYLOAD_COPY_SEMANTIC_VERSION_V17,
            HCOMM_PAYLOAD_COPY_SEMANTIC_VERSION_V18,
            HCOMM_PAYLOAD_COPY_SEMANTIC_VERSION_V19,
        ...
        if json_exists and json_path is not None:
            ...
            found_payload_semantic_v14_marker = (
                found_payload_semantic_v14_marker or
                functions_present.get("payload_semantic_v14", False))
            ...
            found_payload_semantic_v17_marker = (
                found_payload_semantic_v17_marker or
                functions_present.get("payload_semantic_v17", False))
            found_payload_semantic_v18_marker = (
                found_payload_semantic_v18_marker or
                functions_present.get("payload_semantic_v18", False))
            found_payload_semantic_v19_marker = (
                found_payload_semantic_v19_marker or
                functions_present.get("payload_semantic_v19", False))
            ...
            found_payload_official_p2p_layout_marker = (
                found_payload_official_p2p_layout_marker or
                functions_present.get("payload_official_p2p_layout", False))
            ...
            if symbol_state == "present":
                ...
                found_payload_semantic_v14_marker = (
                    found_payload_semantic_v14_marker or
                    symbols_present.get(HCOMM_PAYLOAD_COPY_SEMANTIC_VERSION_V14,
                                        False))
                found_payload_semantic_v15_marker = (
                    found_payload_semantic_v15_marker or
                ...
                found_payload_semantic_v17_marker = (
                    found_payload_semantic_v17_marker or
                    symbols_present.get(HCOMM_PAYLOAD_COPY_SEMANTIC_VERSION_V17,
                                        False))
                found_payload_semantic_v18_marker = (
                    found_payload_semantic_v18_marker or
                    symbols_present.get(HCOMM_PAYLOAD_COPY_SEMANTIC_VERSION_V18,
                                        False))
                found_payload_semantic_v19_marker = (
                    found_payload_semantic_v19_marker or
                    symbols_present.get(HCOMM_PAYLOAD_COPY_SEMANTIC_VERSION_V19,
                                        False))
                found_payload_requires_comm_acquire_marker = (
                    found_payload_requires_comm_acquire_marker or
                ...
                print("function_so.payload_semantic_version_v14."
                      f"{HCOMM_PAYLOAD_COPY_SEMANTIC_VERSION_V14}="
                      f"{'present' if symbols_present.get(HCOMM_PAYLOAD_COPY_SEMANTIC_VERSION_V14, False) else 'missing'}")
                ...
                print("function_so.payload_semantic_version_v17."
                      f"{HCOMM_PAYLOAD_COPY_SEMANTIC_VERSION_V17}="
                      f"{'present' if symbols_present.get(HCOMM_PAYLOAD_COPY_SEMANTIC_VERSION_V17, False) else 'missing'}")
                print("function_so.payload_semantic_version_v18."
                      f"{HCOMM_PAYLOAD_COPY_SEMANTIC_VERSION_V18}="
                      f"{'present' if symbols_present.get(HCOMM_PAYLOAD_COPY_SEMANTIC_VERSION_V18, False) else 'missing'}")
                print("function_so.payload_semantic_version_v19."
                      f"{HCOMM_PAYLOAD_COPY_SEMANTIC_VERSION_V19}="
                      f"{'present' if symbols_present.get(HCOMM_PAYLOAD_COPY_SEMANTIC_VERSION_V19, False) else 'missing'}")
                ...
                print("function_so.payload_official_p2p_layout."
                      f"{HCOMM_PAYLOAD_COPY_SUPPORTS_OFFICIAL_P2P_LAYOUT}="
                      f"{'present' if symbols_present.get(HCOMM_PAYLOAD_COPY_SUPPORTS_OFFICIAL_P2P_LAYOUT, False) else 'missing'}")
        ...
        if symbol_state in ("unreadable", "not-checked"):
            ...
            if args.require_hcomm_payload_kernel:
                required_ok = (
                    ...
                    symbols_present.get(HCOMM_PAYLOAD_COPY_SEMANTIC_VERSION_V17, False) and
                    symbols_present.get(HCOMM_PAYLOAD_COPY_SEMANTIC_VERSION_V18, False) and
                    symbols_present.get(HCOMM_PAYLOAD_COPY_SEMANTIC_VERSION_V19, False) and
        ...
        if args.require_hcomm_payload_kernel:
            ...
            print("required_payload_semantic_v13_symbol="
                  f"{HCOMM_PAYLOAD_COPY_SEMANTIC_VERSION_V13}")
            print("required_payload_semantic_v14_symbol="
                  f"{HCOMM_PAYLOAD_COPY_SEMANTIC_VERSION_V14}")
            print("required_payload_semantic_v15_symbol="
                  f"{HCOMM_PAYLOAD_COPY_SEMANTIC_VERSION_V15}")
            print("required_payload_semantic_v16_symbol="
                  f"{HCOMM_PAYLOAD_COPY_SEMANTIC_VERSION_V16}")
            print("required_payload_semantic_v17_symbol="
                  f"{HCOMM_PAYLOAD_COPY_SEMANTIC_VERSION_V17}")
            print("required_payload_semantic_v18_symbol="
                  f"{HCOMM_PAYLOAD_COPY_SEMANTIC_VERSION_V18}")
            print("required_payload_semantic_v19_symbol="
                  f"{HCOMM_PAYLOAD_COPY_SEMANTIC_VERSION_V19}")
        ...
        if args.require_hcomm_payload_kernel:
            ...
            if found_legacy_payload:
                ...
                print("action=rebuild package with current Flume semantic "
                  ...
                  not found_payload_semantic_v14_marker):
                print("reason=payload kernel package has a stale payload "
                      "semantic marker")
                print("action=rebuild package with current Flume semantic "
                      "v14 remote-entry data-probe-capable payload kernel")
            elif (found_internal_payload_marker and
                  found_payload_abi_version_marker and
                  found_payload_semantic_marker and
                  found_payload_semantic_v5_marker and
                  ...
                  not found_payload_semantic_v15_marker):
                print("reason=payload kernel package has a stale payload "
                      "semantic marker")
                print("action=rebuild package with current Flume semantic "
                      "v15 transfer-exit data-probe-capable payload kernel")
            elif (found_internal_payload_marker and
                  found_payload_abi_version_marker and
                  found_payload_semantic_marker and
                  found_payload_semantic_v5_marker and
                ...
                print("action=rebuild package with current Flume semantic "
                  ...
                  not found_payload_semantic_v17_marker):
                print("reason=payload kernel package has a stale payload "
                      "semantic marker")
                print("action=rebuild package with current Flume semantic "
                      "v17 remote-buffer-safe data-probe-capable payload kernel")
            elif (found_internal_payload_marker and
                  found_payload_abi_version_marker and
                  found_payload_semantic_marker and
                  found_payload_semantic_v5_marker and
                  ...
                  not found_payload_semantic_v18_marker):
                print("reason=payload kernel package has a stale payload "
                      "semantic marker")
                print("action=rebuild package with current Flume semantic "
                      "v18 descriptor-layout-evidence-capable payload kernel")
            elif (found_internal_payload_marker and
                  found_payload_abi_version_marker and
                  found_payload_semantic_marker and
                  found_payload_semantic_v5_marker and
                  ...
                  found_payload_semantic_v16_marker and
                  found_payload_semantic_v17_marker and
                  found_payload_semantic_v18_marker and
                  not found_payload_semantic_v19_marker):
                print("reason=payload kernel package has a stale payload "
                      "semantic marker")
                print("action=rebuild package with current Flume semantic "
                      "v19 descriptor-validation-reason-capable payload kernel")
            elif (found_internal_payload_marker and
                  found_payload_abi_version_marker and
                  found_payload_semantic_marker and
                  found_payload_semantic_v5_marker and
                ...
                print("reason=payload kernel package is missing the "
                      "official-p2p layout capability marker")
                print("action=rebuild package with current Flume "
                      "official-p2p-layout-capable payload kernel")
            elif (found_internal_payload_marker and
                  found_payload_abi_version_marker and
                  found_payload_semantic_marker and
                  found_payload_semantic_v5_marker and
