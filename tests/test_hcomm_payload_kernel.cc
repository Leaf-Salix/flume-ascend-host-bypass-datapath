...
int32_t read_ret = 0;
int32_t write_ret = 0;
int32_t write_with_notify_ret = 0;
...
void* last_write_with_notify_dst = nullptr;
const void* last_write_with_notify_src = nullptr;
uint32_t last_write_with_notify_idx = 0xFFFFFFFFU;
...
void Reset() {
  ...
  last_write_with_notify_dst = nullptr;
  last_write_with_notify_src = nullptr;
  last_write_with_notify_idx = 0xFFFFFFFFU;
...
void CheckTraceHeaderMatchesDesc(
  ...
  FLUME_TEST_CHECK(trace[5] == desc.role);
  FLUME_TEST_CHECK(trace[6] == desc.local_rank);
  FLUME_TEST_CHECK(trace[7] == desc.peer_rank);
  FLUME_TEST_CHECK(trace[8] ==
                   static_cast<uint32_t>(desc.bytes & 0xFFFFFFFFU));
  FLUME_TEST_CHECK(trace[9] ==
                   static_cast<uint32_t>(desc.bytes >> 32U));
  FLUME_TEST_CHECK(trace[10] == static_cast<uint32_t>(desc.reserved2[0]));
  FLUME_TEST_CHECK(trace[11] == static_cast<uint32_t>(desc.reserved2[1]));
  FLUME_TEST_CHECK(trace[12] == desc.completion_mode);
  FLUME_TEST_CHECK(trace[13] == desc.ready_notify_idx);
...
uint32_t WriteNotifyEnterEvent() {
#if FLUME_HAVE_HCOMM_WRITE_WITH_NOTIFY_NBI
  return FLUME_HCOMM_PAYLOAD_TRACE_EVENT_SEND_REMOTE_WRITE_NOTIFY_NBI_ENTER;
#else
  return FLUME_HCOMM_PAYLOAD_TRACE_EVENT_SEND_REMOTE_WRITE_NOTIFY_ENTER;
#endif
}

uint32_t WriteNotifyDoneEvent() {
#if FLUME_HAVE_HCOMM_WRITE_WITH_NOTIFY_NBI
  return FLUME_HCOMM_PAYLOAD_TRACE_EVENT_SEND_REMOTE_WRITE_NOTIFY_NBI_DONE;
#else
  return FLUME_HCOMM_PAYLOAD_TRACE_EVENT_SEND_REMOTE_WRITE_NOTIFY_DONE;
#endif
}
...
int main() {
  ...
  const uint32_t send_remote_entry_fingerprint =
  ...
  FLUME_TEST_CHECK(status[15] == send_remote_entry_fingerprint);
  FLUME_TEST_CHECK(status[16] == PayloadDataFingerprint(local, 16));
  ...
  const uint32_t send_write_local_entry_fingerprint =
      PayloadDataFingerprint(local, 16);
  const uint32_t send_write_remote_entry_fingerprint =
      PayloadDataFingerprint(remote, 16);
  ...
  FLUME_TEST_CHECK(status[13] == PayloadDataFingerprint(user, 16));
  FLUME_TEST_CHECK(status[14] == 16U);
  FLUME_TEST_CHECK(status[15] == send_write_remote_entry_fingerprint);
  FLUME_TEST_CHECK(status[16] == PayloadDataFingerprint(remote, 16));
  CheckTraceHeaderMatchesDesc(send_desc, trace);
  FLUME_TEST_CHECK(std::memcmp(local, user, 16) == 0);
  ...
  const uint32_t send_write_notify_remote_entry_fingerprint =
      PayloadDataFingerprint(remote, 16);
  FLUME_TEST_CHECK(FlumeHcommPayloadCopyDirectAclrtKernelV4(&send_desc) ==
                   FLUME_HCOMM_PAYLOAD_STATUS_SUCCESS);
  FLUME_TEST_CHECK(status[0] == FLUME_HCOMM_PAYLOAD_STATUS_SUCCESS);
  FLUME_TEST_CHECK(status[1] == 0U);
  FLUME_TEST_CHECK(status[15] ==
                   send_write_notify_remote_entry_fingerprint);
  FLUME_TEST_CHECK(status[16] == PayloadDataFingerprint(remote, 16));
  FLUME_TEST_CHECK(std::memcmp(local, user, 16) == 0);
  FLUME_TEST_CHECK(std::memcmp(remote, user, 16) == 0);
  FLUME_TEST_CHECK(last_write_with_notify_dst == remote);
  FLUME_TEST_CHECK(last_write_with_notify_src == local);
  FLUME_TEST_CHECK(last_write_with_notify_idx == send_desc.ready_notify_idx);
  const int send_write_notify_calls[] = {
      kAcquireComm, kBatchStart, kLocalCopy, kWriteWithNotify,
      kNotifyWait, kBatchEnd, kReleaseComm};
  FLUME_TEST_CHECK(CallsEqual(send_write_notify_calls, 7));
  const uint32_t send_write_notify_events[] = {
      FLUME_HCOMM_PAYLOAD_TRACE_EVENT_KERNEL_ENTER,
      FLUME_HCOMM_PAYLOAD_TRACE_EVENT_COMM_ACQUIRE_ENTER,
      FLUME_HCOMM_PAYLOAD_TRACE_EVENT_COMM_ACQUIRE_DONE,
      FLUME_HCOMM_PAYLOAD_TRACE_EVENT_BATCH_START_ENTER,
      ...
      FLUME_HCOMM_PAYLOAD_TRACE_EVENT_SEND_LOCAL_COPY_ENTER,
      FLUME_HCOMM_PAYLOAD_TRACE_EVENT_SEND_LOCAL_COPY_DONE,
      WriteNotifyEnterEvent(),
      WriteNotifyDoneEvent(),
      FLUME_HCOMM_PAYLOAD_TRACE_EVENT_SEND_DONE_WAIT_ENTER,
      FLUME_HCOMM_PAYLOAD_TRACE_EVENT_SEND_DONE_WAIT_DONE,
      FLUME_HCOMM_PAYLOAD_TRACE_EVENT_BATCH_END_ENTER,
      FLUME_HCOMM_PAYLOAD_TRACE_EVENT_BATCH_END_DONE,
  ...
  FLUME_TEST_CHECK(trace[4] == sizeof(send_write_notify_events) /
                                   sizeof(send_write_notify_events[0]));
  for (uint32_t i = 0; i < trace[4]; ++i) {
    FLUME_TEST_CHECK(trace[event_base + i] == send_write_notify_events[i]);
    FLUME_TEST_CHECK(trace[ret_base + i] ==
                     (EventUsesPendingReturn(send_write_notify_events[i]) ?
                          0xFFFFFFFFU :
                          0U));
  }
  ...
  const uint32_t recv_local_entry_fingerprint =
      PayloadDataFingerprint(local, 16);
  const uint32_t recv_remote_entry_fingerprint =
      PayloadDataFingerprint(remote, 16);
  ...
  FLUME_TEST_CHECK(status[13] == PayloadDataFingerprint(user, 16));
  FLUME_TEST_CHECK(status[14] == 16U);
  FLUME_TEST_CHECK(status[15] == recv_remote_entry_fingerprint);
  FLUME_TEST_CHECK(status[16] == PayloadDataFingerprint(local, 16));
  ...
  const uint32_t recv_direct_remote_entry_fingerprint =
      PayloadDataFingerprint(remote, 16);
  FLUME_TEST_CHECK(FlumeHcommPayloadCopyDirectAclrtKernelV4(&recv_desc) ==
                   FLUME_HCOMM_PAYLOAD_STATUS_SUCCESS);
  FLUME_TEST_CHECK(status[0] == FLUME_HCOMM_PAYLOAD_STATUS_SUCCESS);
  FLUME_TEST_CHECK(status[1] == 0U);
  FLUME_TEST_CHECK(status[15] == recv_direct_remote_entry_fingerprint);
  ...
  const uint32_t recv_write_local_entry_fingerprint =
      PayloadDataFingerprint(local, 16);
  const uint32_t recv_write_remote_entry_fingerprint =
      PayloadDataFingerprint(remote, 16);
  FLUME_TEST_CHECK(FlumeHcommPayloadCopyDirectAclrtKernelV4(&recv_desc) ==
                   FLUME_HCOMM_PAYLOAD_STATUS_SUCCESS);
  FLUME_TEST_CHECK(status[0] == FLUME_HCOMM_PAYLOAD_STATUS_SUCCESS);
  ...
  FLUME_TEST_CHECK(status[13] == PayloadDataFingerprint(user, 16));
  FLUME_TEST_CHECK(status[14] == 16U);
  FLUME_TEST_CHECK(status[15] == recv_write_remote_entry_fingerprint);
  FLUME_TEST_CHECK(status[16] == PayloadDataFingerprint(local, 16));
  CheckTraceHeaderMatchesDesc(recv_desc, trace);
  ...
  const int write_notify_enter = FindTraceEvent(
      trace, WriteNotifyEnterEvent());
  const int write_notify_done = FindTraceEvent(
      trace, WriteNotifyDoneEvent());
