...
#define FLUME_HCOMM_PAYLOAD_COPY_SEMANTIC_VERSION 19U
...
#define FLUME_HCOMM_PAYLOAD_COPY_SEMANTIC_VERSION_V19_FUNC \
...
#define FLUME_HCOMM_PAYLOAD_DATA_FINGERPRINT_NOT_SAMPLED 0xFFFFFFFEU
...
#define FLUME_HCOMM_PAYLOAD_VALIDATE_OK 0U
#define FLUME_HCOMM_PAYLOAD_VALIDATE_HEADER 1U
#define FLUME_HCOMM_PAYLOAD_VALIDATE_RANK_SIZE 2U
#define FLUME_HCOMM_PAYLOAD_VALIDATE_ROLE 3U
#define FLUME_HCOMM_PAYLOAD_VALIDATE_LOCAL_RANK 4U
#define FLUME_HCOMM_PAYLOAD_VALIDATE_PEER_RANK 5U
#define FLUME_HCOMM_PAYLOAD_VALIDATE_RANK_PAIR 6U
#define FLUME_HCOMM_PAYLOAD_VALIDATE_NOTIFY_INDEX 7U
#define FLUME_HCOMM_PAYLOAD_VALIDATE_BYTES 8U
#define FLUME_HCOMM_PAYLOAD_VALIDATE_THREAD_NOTIFY_MODE 9U
#define FLUME_HCOMM_PAYLOAD_VALIDATE_COMPLETION_MODE 10U
#define FLUME_HCOMM_PAYLOAD_VALIDATE_AICPU_THREAD 11U
#define FLUME_HCOMM_PAYLOAD_VALIDATE_CHANNEL_HANDLE 12U
#define FLUME_HCOMM_PAYLOAD_VALIDATE_USER_BUFFER 13U
#define FLUME_HCOMM_PAYLOAD_VALIDATE_LOCAL_HCCL_BUFFER 14U
#define FLUME_HCOMM_PAYLOAD_VALIDATE_REMOTE_HCCL_BUFFER 15U
#define FLUME_HCOMM_PAYLOAD_VALIDATE_LOCAL_HCCL_BUFFER_BYTES 16U
#define FLUME_HCOMM_PAYLOAD_VALIDATE_REMOTE_HCCL_BUFFER_BYTES 17U
#define FLUME_HCOMM_PAYLOAD_VALIDATE_STATUS_WORD 18U
#define FLUME_HCOMM_PAYLOAD_VALIDATE_STATUS_WORD_COUNT 19U
#define FLUME_HCOMM_PAYLOAD_VALIDATE_STATUS_SCHEMA 20U
#define FLUME_HCOMM_PAYLOAD_VALIDATE_COMM_NAME 21U
#define FLUME_HCOMM_PAYLOAD_VALIDATE_BATCH_TAG 22U
#define FLUME_HCOMM_PAYLOAD_VALIDATE_CPU_THREAD_ON_AICPU 23U
...
static inline uint32_t flume_hcomm_payload_desc_has_string(
...
static inline uint32_t flume_hcomm_payload_copy_desc_validate_reason(
  ...
  {
    const uint32_t skip_comm_acquire =
        (desc->completion_mode &
         (FLUME_HCOMM_PAYLOAD_COMPLETION_FLAG_SKIP_COMM_ACQUIRE |
          FLUME_HCOMM_PAYLOAD_COMPLETION_FLAG_CHANNEL_HANDLE_BINDING)) != 0U;
    if (!skip_comm_acquire &&
        !flume_hcomm_payload_desc_has_string(
            desc->comm_name, FLUME_HCOMM_PAYLOAD_COMM_NAME_BYTES)) {
      return FLUME_HCOMM_PAYLOAD_VALIDATE_COMM_NAME;
    }
  }
  {
    const uint32_t batch_enabled =
        desc->reserved2[0] != FLUME_HCOMM_PAYLOAD_BATCH_MODE_DISABLED;
    if (batch_enabled &&
        !flume_hcomm_payload_desc_has_string(
            desc->batch_tag, FLUME_HCOMM_PAYLOAD_BATCH_TAG_BYTES)) {
      return FLUME_HCOMM_PAYLOAD_VALIDATE_BATCH_TAG;
    }
  }
  if (desc->thread_notify_mode ==
          FLUME_HCOMM_PAYLOAD_THREAD_NOTIFY_HOST_AICPU &&
      desc->cpu_thread_on_aicpu == 0U) {
