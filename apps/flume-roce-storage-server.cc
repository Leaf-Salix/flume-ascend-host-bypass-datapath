...
bool SendCompletion(int fd, const flume::roce::Completion& completion) {
  std::vector<uint8_t> wire;
  return flume::roce::EncodeCompletion(completion, &wire) && WriteAll(fd, wire.data(), wire.size());
}

int ServeOneClient(int fd, flume::roce::StorageBackend* storage, const std::string& verbs_device,
                   uint32_t timeout_ms) {
  std::vector<uint8_t> wire(flume::roce::kEndpointWireBytes);
  flume::roce::Endpoint peer;
  if (!ReadAll(fd, wire.data(), wire.size()) || !flume::roce::DecodeEndpoint(wire.data(), wire.size(), &peer)) {
    std::cerr << "invalid RoCE peer endpoint received on control connection\n";
    return 1;
  }
  flume::roce::VerbsBackend verbs;
  std::string error;
  if (!verbs.Open(verbs_device, 1, 0, kDefaultQueueDepth, &error) ||
      !verbs.Connect(ToVerbsEndpoint(peer), &error)) {
    std::cerr << "verbs connection setup failed: " << error << "\n";
    return 1;
  }
  if (!flume::roce::EncodeEndpoint(ToWireEndpoint(verbs.endpoint()), &wire) ||
      !WriteAll(fd, wire.data(), wire.size())) {
    std::cerr << "failed to send storage-node RoCE endpoint\n";
    return 1;
  }
  while (true) {
    wire.resize(flume::roce::kCommandWireBytes);
    if (!ReadAll(fd, wire.data(), wire.size())) return 0;
    flume::roce::Command command;
    if (!flume::roce::DecodeCommand(wire.data(), wire.size(), &command)) {
      std::cerr << "invalid RoCE storage command\n";
      return 1;
    }
    flume::roce::Completion completion;
    completion.request_id = command.request_id;
    if (command.object_id != 0 || command.length > static_cast<uint64_t>(SIZE_MAX)) {
      completion.status = 1;
      SendCompletion(fd, completion);
      continue;
    }
    ...
    if (!SendCompletion(fd, completion)) return 1;
...
int main(int argc, char** argv) {
  ...
  for (int index = 1; index < argc; ++index) {
    ...
    if (arg == "--listen" && index + 1 < argc) {
      ...
      std::cerr << "usage: flume-roce-storage-server [--listen ip] "
                ...
                << "[--verbs-device name] [--control-port port] [--timeout-ms ms]\n";
      return 2;
    }
  }
  ...
  std::cout << "flume RoCE storage server scaffold "
            ...
            << "protocol=flume-roce-v1 storage_backend="
  ...
  if (!verbs_device.empty() && control_port == 0) {
    flume::roce::VerbsBackend verbs;
    std::string error;
    if (!verbs.Open(verbs_device, 1, 0, 16, &error)) {
  ...
  if (control_port != 0) {
    ...
    const int result = ServeOneClient(client, storage.get(), verbs_device, timeout_ms);
    close(client);
    return result;
  }
  return 0;
