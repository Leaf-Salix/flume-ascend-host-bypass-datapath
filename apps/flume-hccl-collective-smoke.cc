...
struct RankContext {
  ...
  bool hcomm_aicpu_canary_smoke = false;
  ...
  bool hcomm_payload_write_with_notify = false;
...
const char* OnOff(uint32_t value) {
  return value ? "on" : "off";
}
...
void RankMain(RankContext* ctx) {
  ...
  flume_io_t* hcomm_aicpu_canary_io = nullptr;
  ...
  if (ctx->hcomm_channel_probe) {
    ...
    if (ctx->rank == 0 || ctx->rank == 1) {
      ...
      options.payload_write_with_notify =
          ctx->hcomm_payload_write_with_notify ? 1U : 0U;
  ...
  if (ctx->hcomm_resource_descriptor_smoke) {
    if (ctx->rank_size < 2) {
      error = "HCOMM resource descriptor smoke requires at least two ranks";
      goto cleanup;
    }
    if (ctx->rank == 0 || ctx->rank == 1) {
      uint32_t peer_rank = (ctx->rank == 0) ? 1 : 0;
      ...
      if (!CheckFlume(flume_hcomm_resource_descriptor_smoke_ex(
                      ...
                      "flume_hcomm_resource_descriptor_smoke", &error)) {
        goto cleanup;
      }
      int wait_ret = flume_wait(hcomm_resource_descriptor_io, -1);
      if (wait_ret != FLUME_OK && wait_ret != FLUME_ERR_UNSUPPORTED) {
        error = std::string("flume_wait hcomm resource descriptor smoke "
                            "failed, flume ret=") +
                flume_status_string(wait_ret);
        const char* detail =
            flume_io_error_message(hcomm_resource_descriptor_io);
        if (detail != nullptr && detail[0] != '\0') {
          error += ": ";
          error += detail;
        }
      ...
      line << "rank " << ctx->rank
           ...
           << ": peer_rank=" << peer_rank
           << " usable_hccl_buffer_bytes="
           << flume_io_bytes(hcomm_resource_descriptor_io)
           << " requested_engine=" << HcommEngineName(ctx->hcomm_engine)
           << " resolved_engine="
           << HcommEngineName(ResolveHcommSmokeEngine(ctx->hcomm_engine))
           << " protocol=" << HcommProtocolName(ctx->hcomm_protocol)
  ...
  if (ctx->hcomm_aicpu_canary_smoke) {
    if (ctx->rank_size != 2) {
      error = "HCOMM AICPU canary smoke requires exactly two ranks";
      goto cleanup;
    }
    uint32_t peer_rank = (ctx->rank == 0) ? 1 : 0;
    if (!CheckFlume(flume_hcomm_aicpu_canary_smoke(
                        client, peer_rank, stream, &hcomm_aicpu_canary_io),
                    "flume_hcomm_aicpu_canary_smoke", &error)) {
      goto cleanup;
    }
    int wait_ret = flume_wait(hcomm_aicpu_canary_io, -1);
    if (wait_ret != FLUME_OK) {
      error = std::string("flume_wait HCOMM AICPU canary failed, flume ret=") +
              flume_status_string(wait_ret);
      const char* detail = flume_io_error_message(hcomm_aicpu_canary_io);
      if (detail != nullptr && detail[0] != '\0') {
        error += ": ";
        error += detail;
      }
    ...
    std::ostringstream line;
    line << "rank " << ctx->rank
         << " hcomm standalone AICPU canary passed: peer_rank=" << peer_rank;
    const char* detail = flume_io_error_message(hcomm_aicpu_canary_io);
    if (detail != nullptr && detail[0] != '\0') {
      line << " detail=\"" << detail << "\"";
    }
      ...
      auto log_hcomm_payload_line = [&]() {
        ...
        line << ": peer_rank=" << peer_rank
             ...
             << ((ctx->hcomm_payload_recv_direct_output &&
                  ...
                  !ctx->hcomm_payload_write_with_notify) ? "on" : "off")
             << " payload_channel_fence="
             << (ctx->hcomm_payload_channel_fence ? "on" : "off")
             << " payload_transfer_mode="
             << (ctx->hcomm_payload_write_with_notify ? "write-with-notify" :
...
cleanup:
  ...
  flume_io_release(hcomm_aicpu_canary_io);
...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    } else if (arg == "--hcomm-aicpu-canary-smoke") {
      hcomm_aicpu_canary_smoke = true;
    ...
    } else if (arg == "--hcomm-payload-write-with-notify") {
      hcomm_payload_write_path = true;
      hcomm_payload_write_with_notify = true;
...
#endif
  if (hcomm_require_payload_copy && !hcomm_payload_smoke) {
    std::cerr << "--hcomm-require-payload-copy requires --hcomm-payload-smoke\n";
    return 2;
  }
  ...
  if ((p2p_copy || hcomm_channel_probe || hcomm_custom_op_launch_smoke ||
       hcomm_resource_descriptor_smoke || hcomm_aicpu_canary_smoke ||
  ...
  std::cout << "] ASCEND_RT_VISIBLE_DEVICES="
            ...
            << " hcomm_aicpu_canary_smoke="
            << (hcomm_aicpu_canary_smoke ? "on" : "off")
            << " hcomm_notify_only_smoke="
  ...
  if (flume_get_backend_caps(nullptr, &caps) != FLUME_OK) {
    std::cerr << "flume_get_backend_caps failed\n";
    (void)aclFinalize();
    return 1;
  }
  std::cout << "FLUME_BACKEND_CAPS"
            << " hccl_root_info=" << OnOff(caps.hccl_root_info)
            << " hccl_init_all=" << OnOff(caps.hccl_init_all)
            << " hccl_p2p=" << OnOff(caps.hccl_p2p)
            << " hcomm_channel=" << OnOff(caps.hcomm_channel_res)
            << " hcomm_default_engine="
            << HcommEngineName(caps.hcomm_default_engine)
            << " hcomm_rank_graph=" << OnOff(caps.hcomm_rank_graph)
            << " hcomm_aicpu_thread_export="
            << OnOff(caps.hcomm_thread_export)
            << " hcomm_primitives=" << OnOff(caps.hcomm_primitives)
            ...
            << " storage_hbm="
            << (caps.storage_hbm ? "sim-partial" : "not-implemented")
            << " fallback_hccl_p2p=" << OnOff(caps.fallback_hccl_p2p)
            << " fallback_runtime_staging="
            << OnOff(caps.fallback_runtime_staging)
  ...
  if (hcomm_payload_smoke && global_rank_size < 2) {
    std::cerr << "--hcomm-payload-smoke requires at least two ranks\n";
    (void)aclFinalize();
    return 2;
  }
  ...
  for (uint32_t local_index = 0; local_index < contexts.size(); ++local_index) {
    ...
    contexts[local_index].hcomm_resource_descriptor_smoke =
        hcomm_resource_descriptor_smoke;
    contexts[local_index].hcomm_aicpu_canary_smoke =
        hcomm_aicpu_canary_smoke;
    ...
    contexts[local_index].hcomm_payload_write_path =
        hcomm_payload_write_path;
    contexts[local_index].hcomm_payload_write_with_notify =
        hcomm_payload_write_with_notify;
